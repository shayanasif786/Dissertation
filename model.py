...
from loss import custom_loss, dice_coef, mean_iou
from tensorflow.keras.models import load_model
K.set_image_data_format("channels_last")



def unet_model(input_shape, modified_unet=True, learning_rate=0.0005, start_channel=32, 
    ...
    learning_rate : float
        Learning rate for the model. The default is 0.005.
    start_channel : int
        Number of channels of the first conv. The default is 64 but changed to 32.
    ...
    #     print(f"Loading full model from: {saved_model_dir}")
    #     model = load_model(saved_model_dir, custom_objects={
    #         'combined_loss': custom_loss,
    #         'dice_coef': dice_coef,
    #         'mean_iou': mean_iou
    #     })
    #     return model
    ...
    if modified_unet:
        ...
        x = Conv2D(start_channel, 2, padding='same')(x)  # ✅ now dim matches
    ...
    if saved_model_dir:
        print(f"Loading model weights from: {saved_model_dir}")
...
def level_block_modified(x, dim, level, inc):
    """
    Modified UNet block with SE and residual connections.
    Ensures channel alignment before concatenation.
    """
    if level > 0:
        # Encoder path
        m = res_block(x, dim, encoder_path=True)

        # Downsample with increased filters
        ...
        # 🔧 Ensure channels match before concatenation
        if m.shape[-1] != x.shape[-1]:
            x = Conv2D(int(m.shape[-1]), 1, padding='same')(x)

        # Combine skip connection + decoder
        m = concatenate([m, x])
        m = se_block(m, 8)

        # Decoder path with residual connection
